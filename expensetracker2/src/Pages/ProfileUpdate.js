...
const ProfileUpdate = ({ setProfileComplete }) => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        const response = await fetch(`https://expensetracker2-2f55b-default-rtdb.firebaseio.com/users/${user.uid}.json?auth=${idToken}`);
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      const idToken = await user.getIdToken();
      await fetch(`https://expensetracker2-2f55b-default-rtdb.firebaseio.com/users/${user.uid}.json?auth=${idToken}`, {
        method: 'PATCH',
        headers: {
